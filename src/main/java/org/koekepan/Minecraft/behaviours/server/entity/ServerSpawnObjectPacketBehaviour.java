...
public class ServerSpawnObjectPacketBehaviour implements Behaviour<Packet> {
    private EmulatedClientConnection emulatedClientConnection;
//    private IServerSession serverSession;

    private ServerSpawnObjectPacketBehaviour() {
    }

    public ServerSpawnObjectPacketBehaviour(EmulatedClientConnection emulatedClientConnection) {
...
//        ConsoleIO.println("Block changed at position: " + blockRecord.getPosition().getX() + ", " + blockRecord.getPosition().getY() + ", " + blockRecord.getPosition().getZ());
        ...
        double x = serverSpawnObjectPacket.getX();
        double y = serverSpawnObjectPacket.getY();
        double z = serverSpawnObjectPacket.getZ();

        int entityId = serverSpawnObjectPacket.getEntityId();
